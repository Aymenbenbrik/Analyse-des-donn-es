...
\begin{itemize}
  \item la ligne $i$ représente l'\textit{individu} $\x_i = (x_{i1}, \ldots, x_{ip})^\top$;
  \item la colonne $j$ représente la \textit{variable} $\x^j = (x_{1j}, \ldots, x_{nj})^\top$.
\end{itemize}
\end{definition}
...
Pour la variable $\x^j$ on définit :
\[
...
\[
\Cov(\x^j, \x^k) = \frac{1}{n}\sum_{i=1}^n (x_{ij} - \bar{x}^j)(x_{ik} - \bar{x}^k),
\quad
\Cor(\x^j, \x^k) = \frac{\Cov(\x^j, \x^k)}{s_j s_k}.
\]
\end{definition}

\begin{proposition}[Propriétés de la corrélation]
Pour toutes variables $\x^j, \x^k$ :
\begin{enumerate}
  \item $\Cor(\x^j, \x^k) \in [-1, 1]$ (Cauchy-Schwarz);
  \item $|\Cor| = 1 \Leftrightarrow \x^k$ est fonction affine de $\x^j$;
...
On note $\X_c = \X - \1_n \bar{\x}^\top$ le tableau \textit{centré}. La
...
\begin{proof}
Pour $\X_c$, on a $\bar{\x}_c = 0$, donc $\V(\X_c) = \frac{1}{n}\X_c^\top\X_c = \V(\X)$.
...
positive. La distance associée entre $\x_i$ et $\x_{i'}$ est
\[
d_{\M}^2(\x_i, \x_{i'}) = (\x_i - \x_{i'})^\top \M (\x_i - \x_{i'}).
\]
\end{definition}
...
Soit $\mathcal{N} = \{(\x_i, p_i)\}_{i=1}^n$ avec $\sum_i p_i = 1$ et
$\g = \sum_i p_i \x_i$ le centre de gravité. L'\textit{inertie totale}
de $\mathcal{N}$ par rapport à $\g$ dans la métrique $\M$ est
\[
I_{\mathcal{N}} = \sum_{i=1}^n p_i\, d_{\M}^2(\x_i, \g).
\]
\end{definition}

\begin{proposition}[Inertie et trace]
Avec $\V = \sum_i p_i (\x_i - \g)(\x_i - \g)^\top$,
\[
I_{\mathcal{N}} = \Tr(\V \M).
\]
\end{proposition}
...
\begin{proof}
Par cyclicité de la trace, $(\x_i - \g)^\top \M (\x_i - \g) = \Tr\bigl(\M (\x_i - \g)(\x_i - \g)^\top\bigr)$.
En sommant pondéré sur $i$, $\sum_i p_i \Tr\bigl(\M (\x_i - \g)(\x_i - \g)^\top\bigr) = \Tr(\M \V) = \Tr(\V \M)$.
\end{proof}
...
et de barycentres $\g_k$. Alors
\[
\underbrace{\sum_i p_i\, d^2(\x_i, \g)}_{I_{\text{tot}}}
=
\underbrace{\sum_{k=1}^K \sum_{i \in \mathcal{C}_k} p_i\, d^2(\x_i, \g_k)}_{I_{\text{intra}}}
+
\underbrace{\sum_{k=1}^K P_k\, d^2(\g_k, \g)}_{I_{\text{inter}}}.
\]
\end{theorem}

\begin{proof}
Pour tout $\x_i \in \mathcal{C}_k$, on écrit $\x_i - \g = (\x_i - \g_k) + (\g_k - \g)$.
Le développement de la norme donne
$d^2(\x_i, \g) = d^2(\x_i, \g_k) + d^2(\g_k, \g) + 2\langle \x_i - \g_k, \g_k - \g\rangle_{\M}$.
Par définition du barycentre $\g_k$, le double produit s'annule en sommant sur
...
\[
\mathbf{A} = \U\, \bLambda\, \U^\top = \sum_{k=1}^p \lambda_k\, \u_k \u_k^\top,
\]
...
considère $\u_1$ unitaire maximisant $\u^\top \mathbf{A} \u$ : par compacité
de la sphère, ce maximum existe et vaut $\lambda_1 = \u_1^\top \mathbf{A} \u_1$.
La matrice $\mathbf{A}$ étant symétrique, $\u_1$ est vecteur propre associé.
Sur l'orthogonal $\u_1^\perp$ on applique l'hypothèse de récurrence.
...
Pour $\mathbf{A}$ symétrique et $\u \neq 0$ :
\[
R_{\mathbf{A}}(\u) = \frac{\u^\top \mathbf{A} \u}{\u^\top \u}.
\]
\end{definition}
...
\[
\lambda_1 = \max_{\u \neq 0} R_{\mathbf{A}}(\u),\quad
\lambda_p = \min_{\u \neq 0} R_{\mathbf{A}}(\u),\quad
\lambda_k = \max_{\u \perp \u_1, \ldots, \u_{k-1}} R_{\mathbf{A}}(\u).
\]
\end{theorem}

\begin{proof}
On décompose $\u = \sum_j \alpha_j \u_j$ dans la base orthonormée des vecteurs
propres. Alors $\u^\top \mathbf{A} \u = \sum_j \lambda_j \alpha_j^2$ et
$\u^\top \u = \sum_j \alpha_j^2$. Le quotient s'écrit
\[
R_{\mathbf{A}}(\u) = \frac{\sum_j \lambda_j \alpha_j^2}{\sum_j \alpha_j^2}
\in [\lambda_p, \lambda_1].
\]
Le maximum $\lambda_1$ est atteint pour $\u = \u_1$. Pour le $k$-ième, on impose
...
Soit $\X = \sum_{k=1}^r \sigma_k \u_k \v_k^\top$ la SVD de $\X$. Pour tout
$q \leq r$, la matrice $\X_q = \sum_{k=1}^q \sigma_k \u_k \v_k^\top$ est
...
Les axes $\v_k$ diagonalisent $\X^\top \mathbf{D}_p \X \M$.
\end{theorem}
...
Le premier axe principal $\u_1$ est la direction unitaire maximisant
l'inertie projetée :
\[
\u_1 = \argmax_{\|\u\|=1} \u^\top \V \u.
\]
La solution est le vecteur propre dominant de $\V$ et le maximum vaut $\lambda_1$.
\end{theorem}

\begin{proof}
Lagrangien : $\mathcal{L}(\u, \mu) = \u^\top \V \u - \mu(\u^\top \u - 1)$.
La condition $\nabla_{\u} \mathcal{L} = 2(\V \u - \mu \u) = 0$ donne
$\V \u = \mu \u$, et $\u^\top \V \u = \mu$. Le maximum est atteint pour
$\mu = \lambda_1$ et $\u = \u_1$ (vecteur propre dominant).
\end{proof>

\begin{definition}[Composantes principales]
Le $k$-ième axe principal est $\u_k$ vecteur propre de $\V$ associé à $\lambda_k$.
La $k$-ième composante principale est $\mathbf{c}_k = \X_c \u_k \in \R^n$.
\end{definition>
...
\begin{proof}
(1) Le tableau $\X_c$ est centré donc $\X_c \u_k$ est centré.
(2) $\Var(\X_c \u_k) = \u_k^\top \V \u_k = \lambda_k \u_k^\top \u_k = \lambda_k$.
(3) $\Cov(\X_c \u_k, \X_c \u_l) = \u_k^\top \V \u_l = \lambda_l \u_k^\top \u_l = 0$ (orthogonalité).
...
$F_k(i) = \x_i^\top \u_k$ ; on les représente sur le plan factoriel $(F_1, F_2)$.

\paragraph{Coordonnées des variables (cercle des corrélations) :}

\begin{theorem}
En ACP normée, $G_k(\x^j) = \sqrt{\lambda_k}\, u_{kj} = \Cor(\x^j, \mathbf{c}_k)$,
et $\sum_k G_k(\x^j)^2 \leq 1$.
\end{theorem>

\begin{definition}[Aides à l'interprétation]
Pour l'individu $i$ sur l'axe $k$ :
\[
\cos^2_k(i) = \frac{F_k(i)^2}{\|\x_i - \g\|^2}, \quad
...
$W(\mathcal{C}) = \sum_k \sum_{i \in \mathcal{C}_k} \|\x_i - \g_k\|^2 = n\, I_{\text{intra}}$.
\end{definition>
...
\begin{proof}
À chaque itération :
- l'étape d'affectation diminue $W$ (chaque $\x_i$ est affecté au centre le plus proche);
- l'étape de mise à jour minimise $\sum_{i \in \mathcal{C}_k} \|\x_i - \g\|^2$ en $\g = \g_k$ (centre de gravité).
...
\[
\delta_{\text{Ward}}(A, B) = \frac{|A| |B|}{|A| + |B|}\, d^2(\g_A, \g_B).
\]
\end{definition>
...
\begin{proof}
Avant fusion : inertie intra de $A$ et $B$ égale à $\sum_{x \in A} \|x - \g_A\|^2 + \sum_{x \in B} \|x - \g_B\|^2$.
Après fusion : inertie intra de $A \cup B$ vaut la somme précédente plus
$\frac{|A| |B|}{|A| + |B|} d^2(\g_A, \g_B)$, par développement direct
...
\[
\B = \frac{1}{n}\sum_k n_k (\g_k - \g)(\g_k - \g)^\top, \quad
\W = \frac{1}{n}\sum_k \sum_{i \in \mathcal{C}_k} (\x_i - \g_k)(\x_i - \g_k)^\top.
\]
On a $\V = \B + \W$.
\end{theorem>

\begin{proof}
Pour $\x_i \in \mathcal{C}_k$, $\x_i - \g = (\x_i - \g_k) + (\g_k - \g)$.
Le produit extérieur $(\x_i - \g)(\x_i - \g)^\top$ se développe en :
$(\x_i - \g_k)(\x_i - \g_k)^\top + (\g_k - \g)(\g_k - \g)^\top + 2 \cdot \text{terme croisé}$.
Le terme croisé s'annule par centrage à $\g_k$ en sommant sur $i \in \mathcal{C}_k$,
...
\[
J(\u) = \frac{\u^\top \B \u}{\u^\top \W \u}.
\]
\end{definition>

\begin{theorem}[Solution]
Les axes discriminants maximisant $J$ sont les vecteurs propres généralisés
$\B \u = \mu \W \u$, soit les vecteurs propres de $\W^{-1} \B$. Le maximum
vaut la plus grande valeur propre $\lambda_1$.
\end{theorem>

\begin{proof}
On contraint $\u^\top \W \u = 1$ et on maximise $\u^\top \B \u$ par
Lagrangien : $\mathcal{L} = \u^\top \B \u - \mu(\u^\top \W \u - 1)$.
La condition $\nabla \mathcal{L} = 0$ donne $\B \u = \mu \W \u$. La valeur
...
Sous gaussianité et homoscédasticité, $\hat{k}_B(\x) = \argmin_k (\x - \mu_k)^\top \Sigma^{-1} (\x - \mu_k) - 2 \ln \pi_k$,
